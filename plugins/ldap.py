...
from .common import pass_data, KIND_NAMES, generic_import_actions
...
@click.option('--kind', '-k', type=click.Choice(KIND_NAMES), required=True, help='Kind of data contained in the LDAP query.')
@pass_data
def ldap_import(data, server, port, use_ssl, user, password, base_ou, query, attributes, attr_map, attr_fill, delete_import_fields, kind):
    ...
    all_data = existing_data.cat(query_results)
