...
@click.option('--destination', '-d', help="Name of a data table to store the results of the import operation in.")
@pass_data
def upload_file(data, url, api_key, account_id, data_set_id, remaster_data_set, batch_mode, batch_mode_term_id, users, accounts, terms, courses, sections, enrollments, groups, group_memberships, xlists, user_observers, destination):
    ...
    kwargs['attachment'] = zipfile_path
    try:
        r = c.import_sis_data(account_id, **kwargs)
    except CanvasAPIError as e:
        click.secho('Upload has failed. {}'.format(str(e)), fg='red')
    else:
        if destination is not None:
            data.set(destination, petl.fromdicts([r]))
        r.update({
                 'data_set_name': data_set_name,
                 'data_set_path': app_dir,
                 'used_data_tables': used_data_tables
                 })
