...
@unsync.command()
@click.option('--courses', required=True, type=unicode, help='The source courses to filter.')
...
@pass_data
def account_lookup(data, courses, course_id_field, filter_patterns, filter_pattern_field, filter_course_ids, filter_course_id_field:
    ...
    if filter_patterns:
        for filter_pattern in filter_patterns.data():
    ...
    if filter_course_ids:
        course_ids_to_filter = [i[filter_course_id_field] for i in filter_course_ids.data()]
