...
@click.option('--default-lifecycle-value', default='TERM', type=click.Choice(['TERM', 'SEM', 'ROT', 'YEAR']), help='The default lifecycle to assign all courses. If not changed in the lifecycle-defaults table or the lifecycle-mappings table this will stick.')
...
def add_course_lifecycle(data, courses, course_id_field, default_lifecycle_value, lifecycle_defaults, lifecycle_mappings):
    ...
    lifecycle_default_lookup = dict([(re.compile(i[0]), i[1]) for i in lifecycle_default_lookup])
    ...
    lifecycle_mapping_lookup = dict(lifecycle_mapping_lookup)
