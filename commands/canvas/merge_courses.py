"""Canvas specific command to merge specified courses together."""

import click
from lib.common import pass_data


@click.command()
@click.option('--courses', required=True, type=unicode, help='The source courses table containing courses to be merged.')
@click.option('--course-id-field', required=True, type=unicode, default='course_id', help='The field name for the courses id. Defaults to Canvas standard course_id.')
@click.option('--course-name-field', required=True, type=unicode, default='long_name', help='The field name for the course name. Defaults to Canvas standard long_name.')
@click.option('--merge-data', required=True, type=unicode, help='Table containing pairs of Merged Name -> Source Name mappings.')
@pass_data
def merge_courses(data, courses, course_id_field, course_name_field, merge_data):
    """Merge courses acording to pairs in the merge table."""
    merge_data = data.get(merge_data)
    courses_table = data.get(courses)

    # Turn the merge data into a dict where source_course_name is the key and merged_course_name is the value.
    merge_mappings = merge_data.lookupone('source_id', 'merged_id')

    # Work out which source courses need to be removed and remove them
    source_courses_to_remove = courses_table.select(course_id_field, lambda v: v in merge_mappings)
    source_courses_to_remove = source_courses_to_remove.columns()[course_id_field]
    courses_table = courses_table.select(course_id_field, lambda v: v not in source_courses_to_remove)

    # Work out which merge courses I need to create, this will be any merge course that has a source course to be removed
    merge_courses_to_create = merge_data.select('source_id', lambda v: v in source_courses_to_remove)
    new_merge_course_data = merge_courses_to_create.cut('merged_id', 'merged_name')
    new_merge_course_data = new_merge_course_data.rename({'merged_id': course_id_field, 'merged_name': course_name_field})
    new_merge_course_data = new_merge_course_data.distinct()
    courses_table = courses_table.cat(new_merge_course_data)

    data.set(courses, courses_table)

command = merge_courses
